class GameOverScreen {
    constructor(canvas, type, restartGameCallback, exitGameCallback) {
      ...
      else { this.gameOverImage.src = 'assets/img/3_enemies_chicken/chicken_small/1_walk/1_w.png'; console.log('kann nicht passieren ist aber passiert, nicht von character und endboss'); }
    ...
    handleClick(e) {
      ...
      this.buttons.forEach(b => { if (x >= b.x && x <= b.x + b.width && y >= b.y && y <= b.y + b.height) { console.log('Button clicked:', b.label); b.action(); } });
    }
    handleTouchStart(e) {
      e.preventDefault();
      ...
      let clicked = false;
      this.buttons.forEach(b => { if (x >= b.x && x <= b.x + b.width && y >= b.y && y <= b.y + b.height) { console.log('Button touched:', b.label); clicked = true; b.action(); } });
      if (!clicked) this.handleClick({ clientX: t.clientX, clientY: t.clientY });
    }
    handleTouchEnd(e) {
      e.preventDefault();
      const t = e.changedTouches[0], { x, y } = this.getScaledCoordinates(t.clientX, t.clientY);
      console.log('Touch end detected at:', x, y);
      let clicked = false;
      this.buttons.forEach(b => { if (x >= b.x && x <= b.x + b.width && y >= b.y && y <= b.y + b.height) { console.log('Button released:', b.label); clicked = true; b.action(); } });
      if (!clicked) this.handleClick({ clientX: t.clientX, clientY: t.clientY });
    }
    addEventListeners() {
